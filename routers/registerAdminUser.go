...
// RegisterAdminUser es la funcion que nos permite registrar un nuevo usuario en la base de datos
func RegisterAdminUser(w http.ResponseWriter, r *http.Request) {
	...
	_,exist,_ := db.CheckEmailAdmin(user.Email)
	...
	_,status, err := db.RegisterAdmin(user)
